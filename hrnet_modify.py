...
class HRNetV2(nn.Module):
    def __init__(self, n_class, **kwargs):
        ...
        #self.carafe1 = CARAFE(c=96, scale=2)
        #self.carafe2 = CARAFE(c=192, scale=4)
        #self.carafe3 = CARAFE(c=384, scale=8)
        #self.carafe1 = CARAFE(kernel_size=5, group_size=1, scale_factor=2)
        #self.carafe2 = CARAFE(kernel_size=5, group_size=1, scale_factor=4)
        #self.carafe3 = CARAFE(kernel_size=5, group_size=1, scale_factor=8)
    ...
    def forward(self, x, return_feature_maps=False):
        ...
        #y0 = F.interpolate(y_list[1], size=(400, 400), mode='bilinear', align_corners=False)
        ...
        #print(len(y_list))
        #y0 = F.interpolate(y_list[2], size=(400, 400), mode='bilinear', align_corners=False)
        '''
        x_list = []
...
class hrnetv2(nn.Module):
    def __init__(self, pretrained=False, n_class=1000, decoder='C1', use_softmax=False):
        super(hrnetv2, self).__init__()
        self.encoder = HRNetV2(n_class=n_class)
        fc_dim = 720
        if pretrained:
        ...
        if decoder == 'C1':
            self.decoder = C1(num_class=n_class, fc_dim=fc_dim, use_softmax=use_softmax)
        else:
            self.decoder = nn.Sequential(nn.Conv2d(fc_dim, fc_dim, 1, 1, 0),
                                         BatchNorm2d(fc_dim, BN_MOMENTUM),
                                         nn.ReLU(True),
                                         nn.Conv2d(fc_dim, n_class, 1, 1, 0))
    def forward(self, x):
        segSize = x.size()[-1]
        x = self.encoder(x)
        # x = self.decoder(x, segSize)
        x = self.decoder(x[0])
        x = F.interpolate(x, (segSize, segSize), mode='bilinear')
        return x
'''
...
encoder = hrnetv2()
fMap = encoder(torch.randn(1,3,336,336))
print(fMap[0].size())
decoder = C1(num_class=1, fc_dim=fMap[0].size()[1], use_softmax=True)
output = decoder(fMap, 224)
print(output.size())
'''