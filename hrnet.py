...
class hrnetv2(nn.Module):
    def __init__(self, pretrained=False, n_class=1, decoder='C1', use_softmax=False):
        super(hrnetv2, self).__init__()
    ...
    def forward(self, x):
        segSize = x.size()[-1]
        x, fmap = self.encoder(x)
        # x = self.decoder(x, segSize)
        x_p = self.decoder(x[0])
        #print(x.shape)
        x = F.interpolate(x_p, (segSize, segSize), mode='bilinear')
        fmap = F.interpolate(fmap, (segSize, segSize), mode='bilinear')
        #print(fmap.shape)
        return x, fmap
'''
